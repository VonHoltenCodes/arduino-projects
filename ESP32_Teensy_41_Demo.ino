  ...
  if (Serial2.available()) {
    ...
    // WiFi connect command (format: W<ssid>|<password>)
      ...
      if (separatorIndex != -1) {
        ...
        // Password is now handled securely without being stored in variables that could be exposed
        
        Serial.println("WiFi connection request received");
        
        // Connect to WiFi with credentials from serial (not logging sensitive data)
        WiFi.begin(ssid.c_str(), details.substring(separatorIndex + 1).c_str());
